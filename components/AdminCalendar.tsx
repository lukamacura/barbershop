    ...
    <div className="admin-dashboard mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <header className="mb-12 flex flex-wrap items-center justify-between gap-6 border-b border-white/10 pb-8">
        <div>
          ...
          <span className="mt-3 block h-[3px] w-16 bg-white/70 origin-left" />
        ...
        <div className="flex flex-wrap items-center gap-3">
          <a href="/admin/availability"
            className="flex items-center gap-2 min-h-[44px] rounded-full border-2 border-white/20 bg-transparent px-6 py-2.5 text-[11px] font-bold tracking-[0.15em] uppercase text-white transition-all duration-300 hover:border-[#525252] hover:bg-[#404040] hover:text-white focus-ring">
      ...
      <div className="flex flex-col gap-8 lg:flex-row lg:gap-10">
        ...
        <aside className="shrink-0 lg:w-80">
          <div className="sticky top-6 space-y-6">
            ...
            <div className="rounded-[20px] border border-white/10 bg-[#1a1a1a] p-6">
              <label className="mb-4 block font-heading text-[13px] uppercase tracking-widest text-white/80">Filtriraj po berberu</label>
              <select value={barberFilter} onChange={(e) => updateParams({ barber: e.target.value })}
                className="w-full min-h-[48px] rounded-full border-2 border-white/20 bg-[#141417] px-5 py-3 text-sm font-medium text-white transition-all duration-300 focus:border-[#525252] focus:outline-none focus:ring-2 focus:ring-[#525252]/50">
                <option value="all">Svi berberi</option>
                {barbers.map((b) => (<option key={b.id} value={String(b.id)}>{b.name}</option>))}
              </select>
            </div>

            <div className="flex items-center gap-2 rounded-[20px] border border-white/10 bg-[#1a1a1a] p-2">
              <button type="button" onClick={prevDay}
                className="flex flex-1 items-center justify-center gap-2 min-h-[48px] rounded-full border-2 border-white/20 bg-transparent py-2.5 text-[11px] font-bold tracking-[0.15em] uppercase text-white transition-all duration-300 hover:border-[#525252] hover:bg-[#404040] hover:text-white focus-ring">
        ...
        <main className="min-w-0 flex-1">
          ...
          <div className="admin-schedule-card overflow-hidden rounded-[20px] border border-white/10 bg-[#141417] shadow-xl">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 bg-[#141417] px-8 py-6">
              <div>
              ...
              <span className="rounded-full bg-[#404040] px-5 py-2 text-[12px] font-black tracking-wider text-white uppercase">
            ...
            <div className="overflow-x-auto bg-[#141417]">
          ...
          <div className="admin-modal-content w-full max-w-lg rounded-[24px] border-2 border-white/10 bg-[#1a1a1a] p-8 shadow-2xl"
            ...
            <div className="mb-8 flex items-start justify-between border-b border-white/10 pb-6">
              <div>
                ...
                <span className="mt-2 block h-[3px] w-16 bg-white/70 origin-left" />
            ...
            <dl className="space-y-6">
              <div>
                <dt className="mb-2 font-heading text-[10px] uppercase tracking-widest text-white/70">Korisnik</dt>
                <dd className="text-[15px] font-medium text-white">{selectedReservation.r.customer_name}</dd>
              </div>
              <div>
                <dt className="mb-2 font-heading text-[10px] uppercase tracking-widest text-white/70">Telefon</dt>
              ...
              {selectedReservation.r.customer_email && (
                <div>
                  <dt className="mb-2 font-heading text-[10px] uppercase tracking-widest text-white/70">Imejl</dt>
                  <dd className="text-[15px] text-white">
                    <a href={`mailto:${selectedReservation.r.customer_email}`} className="text-white/90 hover:text-white hover:underline transition-colors">{selectedReservation.r.customer_email}</a>
                  </dd>
                </div>
              )}
              <div className="flex gap-8 pt-2 border-t border-white/10">
                <div>
                  <dt className="mb-2 font-heading text-[10px] uppercase tracking-widest text-white/70">Berber</dt>
                  <dd className="text-[15px] font-medium text-white">{selectedReservation.barber?.name ?? "—"}</dd>
                </div>
                <div className="flex-1">
                  <dt className="mb-2 font-heading text-[10px] uppercase tracking-widest text-white/70">Usluge</dt>
                  <dd className="text-[15px] text-white">
                    {selectedReservation.services.length > 0
                      ? selectedReservation.services.map((s) => s.service_name).join(", ")
                      : "—"}
                    {selectedReservation.services.length > 0 && (
                      <span className="ml-2 font-bold text-white/90">({SLOT_DURATION} min · {selectedReservation.totalPriceRsd} RSD)</span>
                    )}
                  </dd>
                </div>
              </div>
